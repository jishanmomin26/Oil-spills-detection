...
export const AisReconstructionPanel = () => {
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fbbf24', marginBottom: '16px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '12px' }}>
                <span style={{ fontSize: '16px' }}>&#8767;</span>
                <h2 style={{ margin: 0, fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1.5px' }}>AIS Reconstruction</h2>
            </div>

            <div style={{ marginBottom: '20px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    <InfoBox label="Total Vessels" value={reconstruction.total_vessels} />
                    <InfoBox label="Gaps Detected" value={reconstruction.vessels_with_gaps} highlight />
                    <InfoBox label="Total Points" value={reconstruction.total_points} />
                    <InfoBox label="Reconstructed" value={reconstruction.reconstructed_points} highlight />
                </div>
            </div>
            ...
            <div style={{ marginBottom: '8px', fontSize: '11px', color: '#a1a1aa' }}>
                ...
                <div style={{ height: '4px', background: '#27272a', borderRadius: '2px', overflow: 'hidden', marginTop: '6px' }}>
                    <div style={{
                        height: '100%',
            ...
            <div style={{ fontSize: '10px', color: '#71717a', fontStyle: 'italic', lineHeight: 1.4, marginTop: '16px' }}>
                Linear interpolation applied to vessels with detected AIS transmission gaps. Reconstructed points are highlighted in orange on the map.
            </div>
        </div>
    );
};
...
const InfoBox = ({ label, value, highlight = false }: { label: string; value: string | number; highlight?: boolean }) => (
    <div style={{
        ...
        <div style={{ fontSize: '13px', fontWeight: 600, color: highlight ? '#fbbf24' : '#e4e4e7', fontFamily: 'monospace' }}>{value}</div>
    </div>
);
