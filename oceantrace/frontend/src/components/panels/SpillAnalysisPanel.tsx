...
export const SpillAnalysisPanel = () => {
  ...
  const lookalikeFactors = [
    ...
    {
      name: "Rain / Weather Artifact",
      prob: spill.rain_artifact_probability ?? 0,
    },
    ...
    >
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        >
          <InfoBox
            label="Area (km²)"
            value={(spill.area_km2 ?? 0).toFixed(2)}
          />
          <InfoBox
            label="Confidence"
            value={`${((spill.confidence ?? 0) * 100).toFixed(1)}%`}
          />
          <InfoBox
            label="Severity"
            value={spill.estimated_severity ?? "UNKNOWN"}
            highlight={
          ...
          <InfoBox
            label="Est. Age"
            value={`${(spill.estimated_age_hours ?? 0).toFixed(1)} hrs`}
          />
        </div>
      </div>
              ...
              >
                ...
                <span
                  style={{
                    ...
                    color: factor.prob > 0.3 ? "#fbbf24" : "#71717a",
                  }}
                >
      ...
      >
        Note: System identifies look-alike factors but does not definitively
        prove spill reality. Manual verification required. [DEMO DATA]
      </div>
    </div>
  );
};
