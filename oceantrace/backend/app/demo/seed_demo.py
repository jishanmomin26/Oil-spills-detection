...
def wkt_polygon_irregular(lon: float, lat: float, radius_deg: float) -> str:
    """Generate an irregular polygon (realistic spill shape) with guaranteed closed ring."""
    coords = []
    n = 24
    for i in range(n):
        angle = 2.0 * math.pi * i / n
        r = radius_deg * (0.7 + 0.3 * random.random())
        px = lon + r * math.cos(angle) * 1.3  # elongated
        py = lat + r * math.sin(angle)
        coords.append(f"{px:.6f} {py:.6f}")
    coords.append(coords[0])  # ensure exactly closed ring
    return f"POLYGON(({', '.join(coords)}))"
...
def _seed_satellite(db):
    print("  -> Seeding synthetic demonstration satellite observations (3 sequential passes)...")
    
    # PASS 1 — T0 - 12h: Sentinel-1A SAR C-band IW, initial dark anomaly
    prov1_id = uid()
    db.add(DataProvenance(
        id=prov1_id,
        source_type="satellite",
        source_name="Sentinel-1A (Demo)",
    ...
    db.add(SatelliteObservation(
        ...
        sensor="SAR-C / IW",
        acquisition_time=NOW - timedelta(hours=12),
        resolution_m=10.0,
        bounds_wkt="POLYGON((64.4 14.4, 66.4 14.4, 66.4 16.4, 64.4 16.4, 64.4 14.4))",
        processing_status="COMPLETED",
        provenance_id=prov1_id,
    ))

    # PASS 2 — T0: Sentinel-1B SAR, primary detection (backward-compatible)
    prov2_id = uid()
    db.add(DataProvenance(
        id=prov2_id,
        source_type="satellite",
        source_name="Sentinel-1B (Demo)",
    ...
    db.add(SatelliteObservation(
        ...
        processing_status="COMPLETED",
        provenance_id=prov2_id,
    ))

    # PASS 3 — T0 + 12h: Sentinel-2 Optical & SAR follow-up observation displaced SSW 195°
    prov3_id = uid()
    db.add(DataProvenance(
        id=prov3_id,
        source_type="satellite",
        source_name="Sentinel-2 (Demo)",
        source_timestamp=NOW + timedelta(hours=12),
        algorithm_name="prototype_multispectral_sar_processor",
        algorithm_version="1.0.0-prototype",
        result_type="satellite_observation",
        confidence=0.84,
        is_simulated=True,
    ...
    db.add(SatelliteObservation(
        ...
        acquisition_time=NOW + timedelta(hours=12),
        resolution_m=20.0,
        bounds_wkt="POLYGON((64.6 14.3, 66.6 14.3, 66.6 16.3, 64.6 16.3, 64.6 14.3))",
        processing_status="COMPLETED",
        provenance_id=prov3_id,
    ))


def _seed_spill(db):
    print("  -> Seeding oil spills (3 sequential passes corresponding to satellite observations)...")
    
    # PASS 2 SPILL (Primary detection at T0, 18.6 km² - inserted first for backward compatibility)
    random.seed(SEED + 1)
    spill_geom_pass2 = wkt_polygon_irregular(65.500, 15.590, 0.0241)
    prov2_id = uid()
    db.add(DataProvenance(
        id=prov2_id,
    ...
    db.add(OilSpill(
        id="spill-pass-002",
        incident_id=INCIDENT_ID,
        observation_id="sat-obs-002",
        geometry_wkt=spill_geom_pass2,
        centroid_wkt=wkt_point(65.500, 15.590),
        area_km2=18.6,
        perimeter_km=22.4,
        length_km=7.8,
        width_km=2.4,
    ...
    # PASS 1 SPILL (Initial dark anomaly at T0 - 12h, 9.4 km², up-drift toward origin)
    random.seed(SEED + 2)
    spill_geom_pass1 = wkt_polygon_irregular(65.490, 15.585, 0.0165)
    prov1_id = uid()
    db.add(DataProvenance(
        id=prov1_id,
        source_type="algorithm",
        source_name="prototype_spill_detector",
        algorithm_name="c_band_sar_segmentation",
        algorithm_version="1.0.0-prototype",
        result_type="spill_detection",
        confidence=0.88,
        is_simulated=True,
    ...
    db.add(OilSpill(
        id="spill-pass-001",
        incident_id=INCIDENT_ID,
        observation_id="sat-obs-001",
        geometry_wkt=spill_geom_pass1,
        centroid_wkt=wkt_point(65.490, 15.585),
        area_km2=9.4,
        perimeter_km=14.2,
        length_km=4.8,
        width_km=1.8,
        aspect_ratio=2.67,
        orientation_deg=55.0,
        compactness=0.52,
        estimated_age_hours=6.0,
        confidence=0.88,
        detection_method="c_band_sar_segmentation",
        oil_probability=0.88,
        low_wind_probability=0.05,
        ship_wake_probability=0.03,
        rain_artifact_probability=0.03,
        biological_film_probability=0.01,
        provenance_id=prov1_id,
    ))

    # PASS 3 SPILL (Follow-up observation at T0 + 12h, 25.2 km², displaced along SSW 195° prevailing current)
    random.seed(SEED + 3)
    spill_geom_pass3 = wkt_polygon_irregular(65.464, 15.460, 0.0281)
    prov3_id = uid()
    db.add(DataProvenance(
        id=prov3_id,
        source_type="algorithm",
        source_name="prototype_spill_detector",
        algorithm_name="multispectral_sar_segmentation",
        algorithm_version="1.0.0-prototype",
        result_type="spill_detection",
        confidence=0.84,
        is_simulated=True,
    ...
    db.add(OilSpill(
        id="spill-pass-003",
        incident_id=INCIDENT_ID,
        observation_id="sat-obs-003",
        geometry_wkt=spill_geom_pass3,
        centroid_wkt=wkt_point(65.464, 15.460),
        area_km2=25.2,
        perimeter_km=29.8,
        length_km=10.4,
        width_km=3.2,
        aspect_ratio=3.25,
        orientation_deg=66.0,
        compactness=0.42,
        estimated_age_hours=30.0,
        confidence=0.84,
        detection_method="multispectral_sar_segmentation",
        oil_probability=0.84,
        low_wind_probability=0.07,
        ship_wake_probability=0.04,
        rain_artifact_probability=0.03,
        biological_film_probability=0.02,
        provenance_id=prov3_id,
    ))
...
def _seed_vessels(db):
    ...
    cand_names_set = {cfg["name"] for cfg in candidates_config}
    available_non_cand_names = [n for n in VESSEL_NAMES if n not in cand_names_set]

    # ── Generate remaining vessels (not candidates) ──
    for i in range(len(candidates_config), num_vessels):
        v_id = uid()
        mmsi = f"2{i:03d}{random.randint(10000, 99999)}"
        name_idx = i - len(candidates_config)
        name = available_non_cand_names[name_idx] if name_idx < len(available_non_cand_names) else f"MV DEMO-{i:03d}"
